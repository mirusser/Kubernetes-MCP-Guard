...
{
    public const string RedactedValue = "[redacted: prompt-injection-risk]";
    ...
    [
        ...
        (
            "tool-use",
            new Regex(
                @"\b(?:call|invoke|use|run|execute)\b.{0,80}\b(?:tools?|functions?|mcp|apply_approved_plan|request_[a-z_]+)\b|\b(?:apply_approved_plan|request_[a-z_]+)\b.{0,80}\b(?:call|invoke|use|run|execute)\b",
                RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.CultureInvariant)),
        (
            "secret-exfiltration",
            new Regex(
    ...
    public ResponseSanitizationResult SanitizeResponse(string responseText)
    {
        ...
        var withoutManifest = ManifestBlockRegex().Replace(
            ...
            match =>
            {
                ...
                AddTextFindings(match.Groups["manifest"].Value, "response.manifest", findings);

                return $"{match.Groups["prefix"].Value}[redacted: inspect the pending plan file before approval]";
            });
        ...
        out string redactedText)
    {
        ...
        root = RedactJsonNode(root, "response", findings);
        ...
        out bool lineRedacted)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            var before = findings.Count;
            AddTextFindings(line, $"response.line[{i}]", findings);
    ...
    [GeneratedRegex(
        @"^\s*(?:PlanId|Pending file|Approval file|Plan hash|Next step|Applied plan|Status|Operation|Namespace|Objects|API operations|Rollout|Current status):\b|^\s*Call apply_approved_plan\b|^\s*-\s+(?:apps\/v1|v1)\s+(?:Deployment|Service|ConfigMap)\/\S+\b|^\s*(?:Applied|Deleted|Scaled|Restarted)\s+(?:apps\/v1|v1)\s+(?:Deployment|Service|ConfigMap)\b|^\s*(?:Deployment rollout|No rollout)\b",
