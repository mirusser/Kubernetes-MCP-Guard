...
- The optional secondary (kubernetes-mcp-server) contributes only approved entries from its three-tool catalog to `tools/list`; it never contributes to the Destructive tool set or `request_*` wrapper generation. A rejected or faulted secondary snapshot degrades only that source (see catalog isolation below) — the primary downstream and the Gateway itself are unaffected.
- `GET /healthz` always reports healthy once the process is up (liveness only). `GET /readyz` reports overall readiness from PostgreSQL and the mandatory primary downstream only; the optional secondary's lifecycle (`NotConfigured` / `Starting` / `BackingOff` / `Ready` / `Degraded`) is included as informational detail alongside its process/catalog generation and never fails readiness on its own. The response never includes kubeconfig paths, tokens, process arguments, or raw downstream payloads/exception text.
