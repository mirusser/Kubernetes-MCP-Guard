...
public sealed partial class KubernetesManager
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            return KubernetesManagerHelpers.FormatApiException("Event read failed", ex);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        var validation = KubernetesManagerHelpers.ValidateNamespace(options, namespaceName) ??
            ...
            ValidateBoundedCount(tailLines, KubernetesConventions.MaxLogTailLines, "TailLines");
        if (validation is not null)
        {
            return validation;
        }
...
#pragma warning restore MA0004, CA2007
        ...
        catch (Exception ex)
        {
            ...
            return KubernetesManagerHelpers.FormatApiException("Pod log read failed", ex);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            return KubernetesManagerHelpers.FormatApiException("Resource read failed", ex);
        }
    }
    ...
    private Task<string> ReadResourceSummaryAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (IsKind(normalizedKind, KubernetesConventions.KubernetesResources.Deployment))
        {
            return ReadDeploymentSummaryAsync(namespaceName, name, cancellationToken);
        }

        if (IsKind(normalizedKind, KubernetesConventions.KubernetesResources.ReplicaSet))
        {
            return ReadReplicaSetSummaryAsync(namespaceName, name, cancellationToken);
        }

        if (IsKind(normalizedKind, KubernetesConventions.KubernetesResources.Pod))
        {
            return ReadPodSummaryAsync(namespaceName, name, cancellationToken);
        }

        if (IsKind(normalizedKind, KubernetesConventions.KubernetesResources.Service))
        {
            return ReadServiceSummaryAsync(namespaceName, name, cancellationToken);
        }

        return IsKind(normalizedKind, KubernetesConventions.KubernetesResources.ConfigMap)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            @namespace = namespaceName,
            kind = KubernetesConventions.KubernetesResources.ConfigMap,
    ...
    {
        @namespace = namespaceName,
        kind = KubernetesConventions.KubernetesResources.ReplicaSet,
    ...
    private static string? ValidateResourceKind(string kind)
    {
        ...
        if (IsKind(kind, KubernetesConventions.KubernetesResources.Secret))
        {
    ...
    private static bool IsSupportedResourceSummaryKind(string kind) =>
        IsKind(kind, KubernetesConventions.KubernetesResources.Deployment) ||
        IsKind(kind, KubernetesConventions.KubernetesResources.ReplicaSet) ||
        IsKind(kind, KubernetesConventions.KubernetesResources.Pod) ||
        IsKind(kind, KubernetesConventions.KubernetesResources.Service) ||
        IsKind(kind, KubernetesConventions.KubernetesResources.ConfigMap);
    ...
    private static string UnsupportedResourceKindMessage(string kind) =>
        $"Unsupported resource kind '{kind}'. Supported kinds: {KubernetesConventions.KubernetesResources.SupportedResourceSummaryKindsDescription}.";
