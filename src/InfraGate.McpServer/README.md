...
- `K8sManager.*.cs` contains the behavior behind those tools: status reads, bounded observability reads, request-plan creation, approval elicitation, plan application, rollout waits, and validation helpers.
...
- Observability tools are read-only and bounded. They expose Events, Pod logs, and focused summaries, but not Secret values, ConfigMap values, raw manifests, exec, attach, or port-forward.
