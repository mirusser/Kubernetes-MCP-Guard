...
- `ApprovalConventions.cs` holds all shared constants: environment variable names, on-disk directory names, audit event names, challenge statuses, approval source labels, result reason codes, and diff change types.
...
- `IApprovalChallengeStore`, `IApprovalPreExecutionGate`, `IDomainPlanBuilder`, `IDomainPlanExecutor`, `IToolCaller`, `PlanBuildResult`, and `DomainPlanExecutionResult` define generic approval seams used by the gateway and the domain adapter execution path. Failed result records carry stable `ReasonCode` values so callers and tests do not infer behavior from human-readable messages.
