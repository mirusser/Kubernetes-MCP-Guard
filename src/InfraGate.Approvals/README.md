...
`InfraGate.Approvals` is the shared approval storage, challenge, and audit layer. It defines the on-disk plan lifecycle (pending → approved → applied), approval challenge records with hash binding and expiry, audit event conventions, and the strongly-typed audit payload schema.

## Runtime Flow

- `ApprovalStore.cs` persists pending, approved, and applied plan files under `K8S_MCP_APPROVAL_ROOT`, computes SHA-256 hashes, and writes approval audit events to `audit.jsonl`.
...
- `K8sPlan.cs` / `K8sPlanDiff.cs` / `K8sPlanDryRun.cs` model the plan request, its computed diffs, and the stored dry-run result.
