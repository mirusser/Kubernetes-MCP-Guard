...
- `POST /handoff/proposals` accepts `RemediationProposalBatch` payloads from the Planner and returns `202 Accepted` when the batch fits within the configured concurrency cap.
...
- **Input contract:** `InfraGate.Remediation.Contracts.RemediationProposalBatch`.
...
- **Concurrency:** default cap 64 in-flight watched plans; excess handoff batches are rejected with `429 Too Many Requests`.
- **Watch timeout:** default 900 seconds; the watcher uses repeated short `wait_for_plan_approval` calls until the wall-clock timeout is reached.
