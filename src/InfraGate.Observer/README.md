...
- The Observer also acts as an A2A **server** at `/a2a/observer`. The Planner calls back to report plan progress (**Plan Progress Notifications**) and to request read-only K8s tool executions (**Reverse Context Requests**). See [ADR-0028](../../docs/adr/0028-a2a-bidirectional-observer-planner-channel.md).
...
Eight audit-worthy events are defined in `ObserverAuditEvents`:
...
| `handoff.progress` | Inbound plan progress notification from the Planner (`Analyzing`, `PlanProposed`, `NoAction`, `Failed`) |
