...
The narrated walkthrough explains how to diagnose the failure with the read-only MCP tools, propose a fix as a hash-bound plan, approve it through the Gateway browser UI or the direct-stdio `scripts/approve-plan.sh` helper, apply it, and verify recovery.
