...
- `GatewayApprovalServiceTests.cs`: approval URL creation, same-subject enforcement, dry-run data requirements, hash-drift rejection, denial, and Single-Execution challenge behavior.
- `GatewayToolDispatcherTests.cs`: raw destructive tool refusal, domain-target audit storage, and blocked domain execution without applied markers.
