...
| [`Workflows/ExpiredApprovalTests.cs`](Workflows/ExpiredApprovalTests.cs) | 2 | Focused gateway service | A challenge whose `ExpiresAtUtc` is forced into the past is refused at approve time with a stable reason code and does not issue an Approval Grant. |
...
| [`Workflows/ModifiedPendingPlanTests.cs`](Workflows/ModifiedPendingPlanTests.cs) | 5 | Focused gateway service + file tamper | Mutating the pending plan after the challenge is created but before approval is detected at approve time through the pending-plan-changed reason code. |
