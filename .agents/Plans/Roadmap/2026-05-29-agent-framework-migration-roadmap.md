...
## 5. Enforcing Framework-Level Guardrails ✅ Done (2026-05-30)
...
**Delivered:** New `InfraGate.AgentGuardrails` module: shared guardrail vocabulary, function-invocation middleware (`UseToolCallGuardrail` on `AIAgentBuilder`), and consolidated guardrail/hallucination metric taxonomy. Both Observer and Planner agents get the tool-call guardrail via the shared `ToolCallingAgentFactory` seam, each with an explicit read-only tool-name allow-list. Decision-layer validation (`ValidateExecutor`) re-pointed to the shared guardrail metric, replacing two bespoke Planner counters. `InfraGate.AgentGuardrails` meter registered in both hosts' `AddInfraGateTelemetry` MeterNames for export through the Serilog span bridge and opt-in OTLP. `AgentGuardrailPolicy` registered as singleton per service. Block-and-continue default (no `Terminate`) so a blocked tool call does not abort a legitimate multi-tool run. ADR-0027 written.
