...
| **.NET Local Tools (ReportGenerator, SonarScanner, dotnet-coverage)** | Generate code coverage HTML reports (ReportGenerator), SonarCloud CI analysis (SonarScanner), code coverage collection (dotnet-coverage) | ✅ Installed locally via `.config/dotnet-tools.json` |
...
We use `dotnet-reportgenerator-globaltool` as a local tool to generate HTML coverage reports. Additionally, `dotnet-sonarscanner` and `dotnet-coverage` are installed for SonarCloud CI analysis. All are tracked via `.config/dotnet-tools.json`.
