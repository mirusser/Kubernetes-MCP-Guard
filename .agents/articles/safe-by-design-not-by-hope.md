# Safe by Design, Not by Hope: Why MCP Alone Isn't Enough for Mutation Governance
...
Picture this: you've connected your AI coding assistant to your Kubernetes cluster through an MCP server. The agent can inspect namespaces, check pod logs, read deployment status. So far, so good.
...
The agent reads the ConfigMap. The prompt injection fires. The agent decides to be *helpful*. Does your MCP server stop it?

That question — *how do you let an AI touch production without letting it destroy production?* — is deceptively hard, and the answer is not baked into the MCP specification. Let me explain what MCP does give you, what it deliberately doesn't, and why a reference implementation called **InfraGate** is one of the most interesting attempts at filling the gap.
...
The spec also standardizes **user interaction** via elicitation (including a new `url` mode for sensitive out-of-band flows added in the Nov 2025 release), **long-running operations** via tasks, and the client↔server trust boundary via **OAuth 2.1 authorization** with protected-resource metadata, PKCE, and resource indicators.

So MCP is not "you get a tool name and good luck." It's a real interface contract. The question is whether that contract is sufficient for *mutation governance* in high-stakes infrastructure contexts.

The answer is: not yet. And by design.

---

## The Annotation Problem: "Helpful Hints" Is Not a Security Contract

Here's the sentence in the spec that safety engineers should read twice — slowly:
...
Read that again. The spec's own discoverability mechanism — the thing that says "this tool is destructive" — explicitly disavows enforcement authority. `destructiveHint: true` is closer to a Post-it note than a contract. It might improve the model's understanding. It prevents nothing.
...
Before InfraGate implemented its current out-of-band browser approval model, it tried the obvious thing: **MCP elicitation**.
...
Now, the Nov 2025 spec did add **URL-mode elicitation** as a standard mechanism for pushing sensitive human interactions into a browser flow the MCP client doesn't mediate end-to-end. That's a genuine improvement and addresses part of the structural problem. But the spec still doesn't define what happens *next*: what the browser interaction produces, how it binds to the later `tools/call`, what gets verified before the mutation executes. That gap is what InfraGate actually solves.
...
| Long-running operation support | `ToolExecution.taskSupport`, tasks | ✅ Useful plumbing, not approval semantics |
...
The cleanest summary: **MCP standardizes tool discoverability, not mutation governability.**
...
## Enter InfraGate: Building the Missing Layer

**InfraGate** (open source as [Kubernetes MCP Guard](https://github.com/mirusser/Kubernetes-MCP-Guard)) is a reference implementation that treats the above gaps as a design target. It's not replacing MCP — it's building the *application-layer mutation approval profile* that the spec currently lacks.
...
InfraGate enforces a three-phase flow for every mutation. And crucially: **approval is necessary but not sufficient**.
...
The `planId` is an *opaque workflow handle*. It's not an integrity mechanism. The digests are. This distinction matters: if you tamper with the plan, the next digest computation will catch it, not the ID lookup.
...
The grant is bound to: plan id, requester, approver, source challenge, intent digest, review digest, approval policy, reuse constraints, and expiry. It's not a boolean — it's a signed contract.
...
Most "human in the loop" systems conflate two things: the *approval event* (the moment a human clicks Approve) and the *execution authorization* (the durable permission to execute). InfraGate keeps them strictly separate.
...
If a proposed MCP Mutation Approval Profile tries to pretend that every domain is equally reviewable from flat JSON input schemas, it will become a very elegant way to standardize false confidence. Domain adapters are not an implementation convenience — for high-stakes review, they're currently the only way to turn "here is some JSON" into "here is the operational change you are about to sanction."

---

## What a Proper Mutation Approval Profile Would Look Like

If InfraGate's design were to inform a future MCP Mutation Approval Profile, the minimum profile would need:

**Data model:** A generic Plan Envelope with at least: opaque `planId`, `profile`, `operationType`, requester subject, approval policy, execution reuse policy, validity window, freshness policy, `intentDigest`, `reviewDigest`, review-surface context, and evidence artifact summaries. Almost one-for-one with InfraGate's `docs/mutation-approval-profile.md` and `PlanEnvelope` / `PlanEnvelopeFactory` implementation — a good sign, because it means the idea is already executable, not conference-talk vapor.

**API surface:** Minimal and opinionated: a way to propose or request a mutation plan; retrieve review status; subscribe or poll for status updates; a browser-based review/approve/deny path; a final execute-approved-plan operation that accepts `planId` — *not* a blob of mutable execution arguments. InfraGate already approximates this with `request_*`, `execute_approved_plan`, `get_plan_status`, `wait_for_plan_approval`, the `plan://{planId}/status` MCP resource, and the browser approval routes.
...
**Grant semantics:** Approval must produce a durable Approval Grant, not a boolean. Challenges must have terminal states. Grants must be what execution consumes, bound to digests, requester, approver, policy, and expiry.
...
**MCP standardizes tool discoverability. It does not standardize mutation governability.** The spec tells you a tool's name, schema, and some behavioral hints. It gives you URL-mode elicitation for OOB interaction, tasks for long-running operations, and OAuth for transport identity. These are genuinely useful primitives. They are not a complete safety model for AI-driven infrastructure mutations.

**InfraGate fills that gap with a concrete reference implementation.** It proves that you can build approval-bound MCP mutations with a separable generic core and domain adapters, dual-digest binding, browser-based out-of-band review, durable approval grants, and a pre-execution gate sequence that runs even after human approval. It's honest about being experimental. It's specific about what it enforces versus what's defense-in-depth. It backs every claim with code, not vibes.

The broader opportunity: InfraGate's architecture is a reasonable starting point for a formal **MCP Mutation Approval Profile** — a standard layer between MCP's tool discovery primitives and the approval-bound execution semantics that high-stakes mutations require. MCP's URL-mode elicitation, resources, tasks, and authorization pieces could all support such a profile. What's missing is the normative object model and semantic contract that binds them together.

Until that profile exists, InfraGate is a working answer to the question most MCP server builders don't ask until after their first incident:

*Not "what can this tool do?" — but "what exactly did the human approve, and is the server still allowed to execute it right now?"*

---

*InfraGate is open source as [Kubernetes MCP Guard](https://github.com/mirusser/Kubernetes-MCP-Guard). Built with .NET 10, ASP.NET Core, and the official MCP .NET SDK. The mutation-approval profile design lives in `docs/mutation-approval-profile.md` and `docs/mutation-approval-flow.md`. The security model is in `docs/security-model.md`.*
