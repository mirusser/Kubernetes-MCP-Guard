...
| `INFRA_GATE_OAUTH_REQUIRE_HTTPS_METADATA` | `InfraGate.McpGateway.Auth` | No | `true` | `false` | Controls the HTTPS requirement for OIDC discovery metadata. | `false` is acceptable only for localhost DevIssuer development. Production refuses `false`. |
...
| `DEPLOY_HOST` | GitHub Actions environment variable | Required for remote deploys | None | `infra-gate.example.com` | SSH host for the `development` or `production` GitHub Environment. | Configure separately per environment. The host must have Docker Compose v2, `curl`, and the runtime env file already provisioned. |
| `DEPLOY_USER` | GitHub Actions environment variable | Required for remote deploys | None | `deploy` | SSH user used by `package-docker.yml` deploy jobs. | Grant only the permissions needed to manage the target Compose project. |
| `DEPLOY_PORT` | GitHub Actions environment variable | Required for remote deploys | None | `22` | SSH port used by `package-docker.yml` deploy jobs. | Restrict inbound SSH to expected GitHub Actions runner egress where possible. |
| `DEPLOY_PATH` | GitHub Actions environment variable | Required for remote deploys | None | `/opt/infra-gate` | Remote directory where the workflow copies `compose.yaml` before running Docker Compose. | Use separate paths for development and production when they share a host. |
| `DEPLOY_SSH_KEY` | GitHub Actions environment secret | Required for remote deploys | None | OpenSSH private key | Private key used for SSH and SCP. | Store separately in each GitHub Environment. Do not reuse personal keys. |
| `DEPLOY_KNOWN_HOSTS` | GitHub Actions environment secret | Required for remote deploys | None | `ssh-keyscan` output | Pinned SSH host key entries used with strict host checking. | Update intentionally when rotating host keys; do not disable host checking. |
...
| `TAG` | Docker Compose and smoke test | No for development/demo; required by production Compose | `dev` in development deploy, `latest` in demo release Compose | `v0.1.0` | Image tag used by `deploy/compose/*.yaml`, `deploy/mode-c/compose.release.yaml`, and `scripts/smoke-test-release.sh`. | Use the raw release tag for production deploys; avoid floating tags for production. |
| `INFRA_GATE_GATEWAY_IMAGE` | Docker Compose deploys | No | `ghcr.io/mirusser/kubernetes-mcp-guard-gateway` | `mirusser/kubernetes-mcp-guard-gateway` | Gateway image repository used by remote deployment Compose. | Override only when deploying from Docker Hub or a private mirror. |
