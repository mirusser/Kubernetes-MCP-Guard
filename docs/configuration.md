...
| `InfraGate__Observer__LlmApiKey` | `InfraGate.Observer` | Yes | None | (secret) | LLM provider API key. Never logged. Required for Anthropic and OpenRouter providers. | Use a secret manager in production; env var is development-only. Never commit generated files containing this value. |
...
| `InfraGate__Planner__LlmApiKey` | `InfraGate.Planner` | Yes | None | (secret) | API key for the configured LLM provider. Required for Anthropic and OpenRouter. | Use a secret manager in production; env var is development-only. Never commit generated files containing this value. |
