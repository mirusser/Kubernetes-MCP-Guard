...
| `INFRA_GATE_OBSERVER_LLM_PROVIDER` | `InfraGate.Observer` | No | `anthropic` | `anthropic` | LLM provider for anomaly detection. Supported: `anthropic` (default), `openrouter`. Others (`openai`, `google`, `azure`, `ollama`) are reserved for future implementation. | Use `openrouter` to route via OpenRouter (OpenAI-compatible endpoint, e.g. free DeepSeek models). |
...
| `INFRA_GATE_PLANNER_LLM_PROVIDER` | `InfraGate.Planner` | No | `anthropic` | `anthropic` | LLM provider for remediation decisions. Supported: `anthropic` (default), `openrouter`. Others (`openai`, `google`, `azure`, `ollama`) are reserved for future implementation. | Use `openrouter` to route via OpenRouter (OpenAI-compatible endpoint, e.g. free DeepSeek models). |
