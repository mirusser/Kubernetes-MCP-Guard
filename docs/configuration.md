...
| `K8S_MCP_LOG_PATH` | `InfraGate.McpServer` | No | Unset | `/tmp/mcp-server.log` | Optional file path for MCP server debug logs (append mode, auto-flush). When set, all log output is written to this file in addition to the stderr transport. No file is created when this variable is unset. | Use for diagnosing connectivity issues in containerised deployments; disable in steady-state production to avoid unbounded log growth. |
