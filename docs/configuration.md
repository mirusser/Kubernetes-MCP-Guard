...
| `InfraGate__Gateway__KubernetesMcpServer__Arguments__<index>` | `InfraGate.McpGateway` | Required when the secondary is enabled | None | `__0=--config`; `__1=/etc/infra-gate/kubernetes-mcp-server.toml` | Must contain exactly `--config` followed by the generated TOML path. The Gateway rejects other arguments and sibling `conf.d/*.toml` overrides, then appends the trusted `--kubeconfig` argument. | Mount or generate the TOML as a read-only deployment artifact. The generator fixes `read_only=true` and the curated tool allowlist. |
| `InfraGate__Gateway__KubernetesMcpServer__Kubeconfig` | `InfraGate.McpGateway` | Required when the secondary is enabled | None | `/run/kube/mcp-nginx-demo-viewer.config` | Dedicated viewer kubeconfig passed to the secondary as `KUBECONFIG` and `--kubeconfig`. Startup rejects a missing file, a same normalized path as the primary kubeconfig, or a kubeconfig that is not single-context. | Use only the namespace-scoped viewer credential generated by `scripts/create-demo-kubeconfig.sh`; never reuse the primary mutation-capable kubeconfig. |
| `InfraGate__Gateway__KubernetesMcpServer__Context` | `InfraGate.McpGateway` | Required when the secondary is enabled | None | `minikube-mcp` | Exact current context required in the dedicated single-context kubeconfig and validated at startup. | Keep the kubeconfig single-context and pin this value in the reviewed run profile. |
| `InfraGate__Gateway__KubernetesMcpServer__AllowedNamespaces__<index>` | `InfraGate.McpGateway` | Required when the secondary is enabled | None | `__0=mcp-nginx-demo` | Exact namespace allowlist for secondary requests. Empty and wildcard values fail configuration validation. | Grant matching namespace-scoped viewer RBAC; widen only through review. |
...
> [!NOTE]
> When the secondary is enabled, the Gateway runs a one-time startup validation of the composed bundle before registering it, in addition to the per-field checks in `KubernetesMcpServerProcessOptions.FromConfiguration` above:
>
> - **Executable check**: `Command` must exist and (on non-Windows platforms) be user-executable.
> - **Config check**: the generated TOML path (the second `Arguments` entry) must exist.
> - **Kubeconfig writability check**: on non-Windows platforms, the resolved `Kubeconfig` must not be group- or other-writable.
> - **Version check**: a colocated `kubernetes-mcp-server.manifest.json` must sit next to `Command` in the same directory; the Gateway runs `Command --version` and requires the reported version to exactly match the manifest's pinned `version`. This is why `scripts/install-kubernetes-mcp-server.sh` and the Gateway image both colocate the manifest with the installed binary.
> - **Policy check**: the TOML's `enabled_tools` must exactly match the approved read-only tool set (`pods_list_in_namespace`, `pods_get`, `pods_log`) — no more, no fewer.
>
> Any failed check throws at startup and the secondary is never registered; the Gateway does not fall back to running an unvalidated bundle.
...
**Generated file transport**: `deploy/generated/*.env` and `deploy/generated/*.toml` files are gitignored. The committed no-SDK example is `deploy/local-oauth/release.env.example`, regenerated from the `smoke-release` profile. The TOML generator requires a dedicated `kubernetesMcpServer` profile and fixes the viewer kubeconfig, single-cluster provider, kubeconfig auth, core toolset, stateless mode, redundant destructive-tool denial, and curated `enabled_tools` list.
...
| `INFRA_GATE_REQUIRE_GATEWAY_INTEGRATION` | Test environment | No | Unset, live gateway integration test returns early | `1` | Same opt-in live Kubernetes integration coverage for `InfraGate.McpGateway.Tests` as `INFRA_GATE_RUN_GATEWAY_INTEGRATION`, but fails the test with `Assert.Fail` instead of skipping it when prerequisites (the pinned binary, the viewer kubeconfig) are missing. | Use in CI jobs that must fail loudly rather than silently pass via a skipped test. |
