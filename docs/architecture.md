...
The local/demo source Compose file builds both images locally, and the local/demo release Compose file pulls GHCR images by default while documenting Docker Hub equivalents. The remote Docker host Compose files under `deploy/compose/` deploy the gateway image only with a real OIDC provider. Tags and CI/CD settings are owned by [docs/configuration.md](configuration.md) and the release process docs.
