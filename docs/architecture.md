...
The local source Compose path builds the gateway, Observer, Planner, and Executor images and starts Keycloak, PostgreSQL, and Mailpit for the OAuth-backed demo stack. The published-image and deployment Compose files deploy the gateway image only; development uses the local Keycloak setup script, while production uses a real OIDC provider. Tags and CI/CD settings are owned by [docs/configuration.md](configuration.md) and the release process docs.
