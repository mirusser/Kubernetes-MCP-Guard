#!/usr/bin/env bash
# smoke-test-keycloak-release.sh — Mode D released-image smoke test.
#
# Exercises deploy/mode-d/compose.release.yaml using published gateway images:
...
#   3. Verifies the unauthenticated MCP challenge includes resource_metadata.
#   4. Acquires a real Keycloak token through the local smoke client.
#   5. Confirms a bearer-authenticated /mcp request is not rejected as 401/403.
#
# Usage:
...
COMPOSE_FILE="${REPO_ROOT}/deploy/mode-d/compose.release.yaml"
...
teardown() {
  ...
  if [[ ${exit_code} -ne 0 ]]; then
    echo ""
    echo "FAIL: Keycloak smoke test failed for tag '${TAG}'. Last logs:" >&2
...
echo "==> Pulling and starting Mode D services (tag=${TAG}) ..."
...
echo ""
echo "OK: Keycloak smoke test passed for tag '${TAG}'."
