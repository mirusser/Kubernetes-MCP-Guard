...
- Opt-in file logging for the MCP server via `K8S_MCP_LOG_PATH` environment variable. When set, all log output is written to the specified file (append mode, auto-flush) in addition to the stderr transport. No file is created when the variable is unset.
